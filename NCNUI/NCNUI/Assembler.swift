...
class Assembler {
    ...
    static func getViewSubscriptionView(router _: ViewServiceRouterContract) -> ViewSubscriptionView {
    ...
    static func getCreateavaialabaleSubscription(subscriptionId: Int, subscriptionPackageType: String, subscriptionCountLimit: Float, subscritptionDayLimit _: Int, serviceId: Int) -> CreateAvailableSubscriptionView {
    ...
    static func getEnterpriseNameView(id: Int) -> GetEnterpriseNameView {
        let usecase = getEnterpriseName()
        let presenter = GetEnterpriseNamePresenter(getEnterpriseName: usecase)
        let view = GetEnterpriseNameView(id: id, presenter: presenter)
        view.presenter = presenter
        presenter.view = view
        return view
    }

    private static func getEnterpriseName() -> GetEnterpriseName {
        let network = GetEnterpriseNameNetworkService()
        let datamanager = GetEnterpriseNameDataManager(networkService: network)
        let usecase = GetEnterpriseName(dataManager: datamanager)
        return usecase
    }
